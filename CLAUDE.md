...
- **`README.md`** — single deliverable. Hand-authored HTML+Markdown, centered hero, then About / Tech Stack / Experience / Featured Project. Section order is recruiter-scan-optimized (identity first, keywords second, depth after) — don't reorder without asking.
...
- **One theme for both GitHub modes** — assets ship in the dark navy palette only, as plain `<img>` tags (owner's choice, July 2026: light `-light.svg` twins and `<picture>` blocks were tried and removed). The navy banner reads fine on GitHub's light theme. Don't add per-theme variants or `prefers-color-scheme` sources unless asked.
